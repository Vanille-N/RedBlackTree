    ...
    class Tree(V)
        ...
        def display
            ...
            String.build do |buf|
                buf << NEWLINE
                if (x = @root).is_a? Leaf
                    buf << ROOT << HLINE*3 << ENDING << BLKCIRC << NEWLINE
                else
                    div = [] of Bool
                    lt = x.left
                    rt = x.right
                    if lt.is_a? Leaf && rt.is_a? Leaf
                        buf << ROOT << HLINE*3 << (x.red? ? REDCIRC : BLKCIRC)
                        buf << align(0, h)
                        buf << x.val
                    else
                        display_helper(div, true, buf, 1, h, lt)
                        buf << ROOT << HLINE*3 << NODE << (x.red? ? REDCIRC : BLKCIRC)
                        buf << align(0, h)
                        buf << x.val
                        buf << NEWLINE
                        display_helper(div, false, buf, 1, h, rt)
                    end
                end
                buf << NEWLINE
            end
...
def main
    bst = RedBlackTree::Tree(Int32).new
    ...
    bst = RedBlackTree::Tree({Int32, Char}).new do |a, b|
        if a[0]%2 < b[0]%2
            true
        elsif a[0]%2 > b[0]%2
            false
    ...
    20.times do |k|
        bst << {Random.rand(100), 'a' + Random.rand(26)}
    end
    puts bst.display

    bst.enum_ref do |i, x|
        puts "#{i}'th node is #{x} and has depth #{bst.depth(x)}"
    end
end
